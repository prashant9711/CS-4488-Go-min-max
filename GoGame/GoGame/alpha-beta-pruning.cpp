...
int parallelAlphaBeta(std::shared_ptr<Node> node, int depth, int alpha, int beta, bool maximizingPlayer, steady_clock::time_point startTime) {
    ...
    if (maximizingPlayer && depth != 0) {
        ...
        for (std::shared_ptr<Node> child : node->children) {
            ...
            // Create an asynchronous task and store the future in the 'futures' vector
            futures.push_back(std::async(std::launch::async, [child, dynamicDepth, alpha, beta, startTime]() {
                return alphaBeta(child, dynamicDepth, alpha, beta, false, startTime);
                }));
        }
        ...
        for (std::shared_ptr<Node> child : node->children) {
            ...
            // Create an asynchronous task and store the future in the 'futures' vector
            futures.push_back(std::async(std::launch::async, [child, dynamicDepth, alpha, beta, startTime]() {
                return alphaBeta(child, dynamicDepth, alpha, beta, true, startTime);
                }));
        }
